...
/**
  連続するコマンドをまとめる
*/
export default function optimize(commands: ICommand[]): ICommand[] {
  let prev: ICommand
  ...
  commands.forEach((c, i) => {
    ...
    // 連続していたら前回のコマンドとマージ
    if (prev && c.target === prev.target && c.action === prev.action) {
      if (c.action === "translate") {
        // 値を加算
        prev.options[0] = `${parseFloat(prev.options[0]) +
          parseFloat(c.options[0])}`
        prev.options[1] = `${parseFloat(prev.options[1]) +
          parseFloat(c.options[1])}`
        optimized = true
      }
